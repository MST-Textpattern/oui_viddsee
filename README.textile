h1. oui_player_viddsee

h2. Introduction

"oui_player_viddsee":https://github.com/NicolasGraph/oui_player_viddsee is a "oui_player":https://github.com/NicolasGraph/oui_player v2+ extension to easily embed "Viddsee":https://viddsee.go.com/ provided videos in "Textpattern":https://textpattern.com/ powered websites.
...
oui_player_viddsee's minimum requirements:
...
# "Download":https://github.com/NicolasGraph/oui_player_viddsee/releases the compiled plugin file or the source to compile a customized file.
...
# Install oui_player_viddsee:
@$ composer require oui/oui_player_viddsee@
...
bc. $ composer update oui/oui_player_viddsee
...
bc. $ composer remove oui/oui_player_viddsee

h2. Tags and parameters

h3. oui_viddsee vs oui_player

While the @<txp:oui_player />@ tag is able to embed any video from any extension-related provider, the @<txp:oui_viddsee />@ tag is a shortcut to embed Viddsee videos only. It is strictly equivalent to @<txp:oui_player provider="viddsee" />@.
...
h3. oui_if_viddsee vs oui_if_player

p. While the @<txp:oui_if_player>…</txp:oui_if_player>@ tag is able to check an URL against any URL from any extension-related provider, the @<txp:oui_if_viddsee>…</txp:oui_if_viddsee>@ tag is a shortcut to check an URL against Viddsee related URL schemes only. It is strictly equivalent to @<txp:oui_if_player provider="viddsee">…</txp:oui_if_player>@.
...
"oui_player":https://github.com/NicolasGraph/oui_player preference related field value: @https://www.viddsee.com/video/the-lying-theory/j2zn3@.

h3. Embed a player by defining its provider and a media ID through attributes

bc(language-markup). <txp:oui_player provider="viddsee" play="j2zn3" />

…or:

bc(language-markup). <txp:oui_viddsee play="j2zn3" />

h3. Conditionally embed a player using attributes

bc(language-markup).. <txp:oui_if_player provider="viddsee" play="https://www.viddsee.com/player/j2zn3">
    <txp:oui_player />
</txp:oui_if_player>

p. …or:

bc(language-markup).. <txp:oui_if_viddsee play="https://www.viddsee.com/player/j2zn3">
    <txp:oui_viddsee />
</txp:oui_if_viddsee>

p. …or as a "short tag":https://docs.textpattern.io/tags/tag-basics/core-short-tags (Txp 4.7+):

bc(language-markup).. <oui::if_viddsee play="https://www.viddsee.com/player/j2zn3" />
    <oui::viddsee />
</oui::if_viddsee>
...
oui_player_viddsee version 1.0.0, Copyright (C) 2018 Nicolas Morand
...
* *oui_player_viddsee v1.0.0-beta* (2018-06-24)
